...
while True:
    ...
    # draw box for left (D)
    frame_copy = cv2.rectangle(frame_copy, (width // 2 + windowSize // 2, 0), (width - 2, height // 2), (255, 255, 255),
                               1)
    ...
    frame_copy = cv2.rectangle(frame_copy, (width // 4, ((3 * height) // 4) + 5), (3 * width // 4, height),
                               (255, 255, 255), 1)
    cv2.putText(frame_copy, 'DOWN', ((3 * width // 4) - 100, (height // 2) + 108), cv2.FONT_HERSHEY_SIMPLEX, 1,
                (255, 255, 255))
